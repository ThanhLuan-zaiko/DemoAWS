...
function Products() {
  ...
  const load = async () => setRows(await api("/products"));
  useEffect(() => {
    load();
  }, []);
  ...
  async function edit(r) {
    ...
    if (!name) return;
    const unit = prompt("Unit price:", r.unit_price);
    if (unit == null) return;
    await api("/products/" + r.id, {
      method: "PUT",
      body: JSON.stringify({ sku: r.sku, name, unit_price: Number(unit) || 0 }),
    });
    await load();
  }
          ...
          {rows.map((r) => (
            <tr key={r.id}>
              ...
              <td>
                ...
                <button style={{ marginLeft: 6 }} onClick={() => del(r.id)}>
                  Delete
                </button>
              </td>
            </tr>
...
function Suppliers() {
  ...
  const [form, setForm] = useState(empty);
  const [rows, setRows] = useState([]);
  const load = async () => setRows(await api("/suppliers"));
  useEffect(() => {
    load();
  }, []);
...
function StockMoves() {
  ...
  const load = async () => {
    setMoves(await api("/stock_moves"));
  ...
  return (
    <Section title="Stock Movements">
      ...
      <div style={{ marginTop: 8 }}>
        <button onClick={apply}>Apply Move</button>
      </div>
